...
export class BluGo {
  ...
  constructor(accountData, manager) {
    ...
    get shortId() { return this.accountId.slice(0, 8) }
  ...
  start() {
    ...
    this.client.once('ready', () => {
      ...
      if (this.actions.denyFriendRequests) {
        const pending = this.client.friend?.pendingList?.filter(
          (f) => f.direction === 'INCOMING',
        ) ?? []
        pending.forEach((f) => f.decline?.().catch(() => {}))
        if (pending.length > 0) {
          log(this.accountId, 'info', `Declined ${pending.length} pending friend request(s)`)
        }
      }
      ...
      this.client.party?.me.setOutfit("CID_039_Athena_Commando_F_Disco");
      this.client.party?.me.setBattlePass(true, 999, 100, 100);
      this.client.party?.me.setEmote("EID_Hype");
    })
    ...
    this.client.on('party:member:joined', async (member) => {
      ...
      try {
        ...
        if (state && state !== MatchmakingState.NOT_MATCHMAKING) {
          if (member.id === this.accountId) {
            log(this.accountId, 'warn', 'Party already in matchmaking when joined → leaving immediately')
      ...
      const members = member.party.members
        .map((m) => ({ accountId: m.id, displayName: m.displayName, isLeader: m.isLeader }))
        .filter((m) => m.accountId !== this.accountId)

      bus.emit('joined', { accountId: this.accountId, members })
      log(this.accountId, 'info', `Party members: ${members.map((m) => m.displayName || m.accountId.slice(0, 8)).join(', ')}`)
    })
    ...
    this.client.on('party:invite', async (invitation) => {
      ...
      try {
        ...
        log(this.accountId, 'error', `Error while accepting invite: ${err?.code ?? err?.message}`)
        this._setPresence(Presence.ACTIVE, idleMsg)
      ...
      // Lobby → JoiningGame: el matchmaking acaba de empezar
      if (from === 'Lobby' && to === 'JoiningGame') {
        log(this.accountId, 'ok', `Matchmaking detected → leaving in ${TIMINGS.matchstateLeaveDelayMs}ms`)
  ...
  stop() {
    ...
    if (this.retryCount <= RECONNECT.maxRetries) {
      log(this.accountId, 'warn', `Disconnected — reintento ${this.retryCount}/${RECONNECT.maxRetries}`)
      this._scheduleReconnect()
    } else {
  ...
  _onXmppError(err) {
    ...
    const shouldReconnect = [
      'disconnect', 'invalid_refresh_token', 'party_not_found',
  ...
  async _applyPatch() {
    ...
    const mpLoadout = (() => {
      try { return JSON.parse(schema['Default:MpLoadout_j'] ?? '{}') }
      catch { return {} }
    })()
    ...
    const cosmetics = {
      ...
      'Default:AthenaCosmeticLoadout_j': JSON.stringify({
        AthenaCosmeticLoadout: {
          characterPrimaryAssetId: 'AthenaCharacter:Character_SuperNovaTaro',
        },
      }),
    }
    ...
    if (mpLoadout?.MpLoadout?.d !== undefined) {
      cosmetics['Default:MpLoadout_j'] = JSON.stringify({
        MpLoadout: {
          d: JSON.stringify({ ac: { i: 'Character_SuperNovaTaro', v: [] } }),
        },
      })
    }
    ...
    const patch = {
      'Default:FORTStats_j': JSON.stringify({
        FORTStats: {
          fortitude:             stat, offense:             stat,
          resistance:            stat, tech:                stat,
          teamFortitude:         0,    teamOffense:         0,
          teamResistance:        0,    teamTech:            0,
          fortitude_Phoenix:     stat, offense_Phoenix:     stat,
          resistance_Phoenix:    stat, tech_Phoenix:        stat,
          teamFortitude_Phoenix: 0,    teamOffense_Phoenix: 0,
          teamResistance_Phoenix: 0,   teamTech_Phoenix:    0,
        },
      }),
