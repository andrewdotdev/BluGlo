...
export function startCLI(manager: BotManager): void {
  ...
  rl.on('line', async (line) => {
    ...
    switch (cmd.toLowerCase()) {
      ...
      case '/remove': {
        if (!args[0]) {
          console.log('Usage: /remove <accountId>');
          break;
        }
      ...
      case '/stats': {
        ...
        console.log('');
        for (const bot of bots) {
          const uptime = bot.stats.connectedAt ? Math.floor((Date.now() - bot.stats.connectedAt) / 1000) : 0;
          console.log(
...
function _extractAuthorizationCode(input: string): string | null {
  ...
  if (value.startsWith('{') && value.endsWith('}')) {
    try {
      const data = JSON.parse(value) as { authorizationCode?: string; code?: string };
...
function _question(rl: readline.Interface, text: string): Promise<string> {
